...
function generateTerrain(
  ...
  const finalMap = height.map((line, x) =>
    line.map((t, y) => {
      ...
      return new Tile(
        getTileType(h, seaLevel, mountainLevel, temp, humid, x, y),
        new Position(x, y),
        h
      );
    })
...
function buildRegions(world: World): void {
  ...
  world.cities.forEach((city) => {
    ...
    Utils.getNeighbors(world.map, city.position).forEach((n) => {
      ...
      queue.push({
        t: n,
        score: getMovementScore(world.map[city.position.x][city.position.y], n),
      });
    });
  });
