...
import { City, Faction, IndustryName, Nation, Node, People, Position, Relation, Road, Tile, World } from "./model/models";
...
class WorldSave {
    ...
    public static fromWorld(world: World): WorldSave {
        return new WorldSave(
            Saving.simplifyMap(world.cities).map(([id, city]) => [id, CitySave.fromCity(city)]),
            world.rivers,
            Saving.simplifyMap(world.people).map(([id, people]) => [id, PeopleSave.fromPeople(people)]),
            Saving.simplifyMap(world.factions).map(([id, faction]) => [id, FactionSave.fromFaction(faction)]),
            WorldSave.compressMap(world.map),
            Saving.simplifyMap(world.nations).map(([id, nation]) => [id, NationSave.fromNation(nation)]),
    ...
    public static toWorld(save: WorldSave): World {
        ...
        world.cities = new Map(save.cities.map(([id, city]) => [id, CitySave.toCity(city)]));
        world.people = new Map(save.people.map(([id, people]) => [id, PeopleSave.toPeople(people)]));
        world.factions = new Map(save.factions.map(([id, faction]) => [id, FactionSave.toFaction(faction)]));
        world.nations = new Map(save.nations.map(([id, nation]) => [id, NationSave.toNation(nation)]));
        world.neighbours = new Map(save.neighbours);
        return world;
    }
    public static compressMap(map: Tile[][]): string[][] {
        return map.map(col => col.map(tile => 
            // compress tile info into a single string
            `${tile.isFrontier ? 1 : 0}|${tile.isRoad ? 1 : 0}|${tile.isSeaRoad ? 1 : 0}|${tile.wasHole ? 1 : 0}|${tile.altitude}|${tile.cityId}\
            |${tile.position.x}/${tile.position.y}|${tile.type}|${tile.riverName}|${tile.waterFlow}`
        ));
    }
    public static extractMap(map: string[][]): Tile[][] {
        return map.map(col => col.map(s => {
            // read tile info from string
            const split = s.split('|');
            const pos = split[6].split('/');
            const tile = new Tile(split[7], new Position(Number(pos[0]), Number(pos[1])), Number(split[4]));
...
class CitySave {
    ...
    public static fromCity(city: City): CitySave {
        return new CitySave(
            city.port, city.rivers,
            city.roads.map(road => RoadSave.fromRoad(road)),
            city.nation, city.access, city.stability, city.growth,
            ...
            city.industries, city.position, city.color
        );
    }
    public static toCity(save: CitySave): City {
        const city = new City(save.name, save.population, save.industries, save.position, save.color);
        city.id = save.id;
        city.port = save.port;
...
class RoadSave {
    ...
    public static fromRoad(road: Road): RoadSave {
        return new RoadSave(
            road.id, road.from, road.to, road.path, road.cost
        );
    }
...
class FactionSave {
    ...
    public static fromFaction(faction: Faction): FactionSave {
        return new FactionSave(
            ...
            faction.wealth, faction.members
        );
    }
    public static toFaction(save: FactionSave): Faction {
        const faction = new Faction(
            save.name, new Map(save.reputation),
            save.wealth, save.members
        );
        faction.id = save.id;
        return faction;
    }
...
class PeopleSave {
    ...
    public static fromPeople(people: People): PeopleSave {
        return new PeopleSave(
            ...
            people.relations, people.location
        );
    }
    public static toPeople(save: PeopleSave): People {
        const people = new People(
            save.name, new Map(save.stats),
            new Map(save.reputation), save.relations,
            save.location
        );
        people.id = save.id;
        return people;
...
class NationSave {
    ...
    public static fromNation(nation: Nation): NationSave {
        return new NationSave(nation.id, nation.name, LanguageSave.fromLanguage(nation.lang), nation.color);
    }
